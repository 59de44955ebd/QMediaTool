...
* $"TOOLNAME"             - for any provided tool (see below), so e.g. $FFMPEG for ffmpeg.
...
* rtmpdump                - command-line tool that allows to dump RTMP streams
* vlc                     - shell script that calls and forwards arguments to VLC (which is expected at the default location)
* youtube-dl              - command-line download manager for media files hosted on YouTube and many other video hosters
